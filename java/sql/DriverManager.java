...
public class DriverManager {
    ...
    /**
     * 在我们调用这个类的方法时,静态块中的语句会首先执行，这里是加载我们的jdbc Driver
     *
     *
     *
     *
    ...
    /**
     * 在我们分析源码时,一定要记住当我们主动使用某个类时,首先要看它的父类是非有静态初始化语句,其实再看自身是否有静态初始化语句
     * 当前类没有父类(Object除外),但是我们发现它自身是由初始化语句的,看上面
     *
     *
     *
     * Attempts to establish a connection to the given database URL.
    ...
    public static Connection getConnection(String url,
        ...
        //分析上面之后也就分析一大半的,这里主要是把我们传入的信息做一下判断并封装,便于传参
        ...
        //getCallerClass()就是获取到调用这个方法的Class对象
        return (getConnection(url, info, Reflection.getCallerClass()));
    }
    ...
    public static synchronized void registerDriver(java.sql.Driver driver,
        ...
        if(driver != null) {
            // 如果没有就会缓存到我们的registeredDrivers中
    ...
    private static void loadInitialDrivers() {
        ...
        AccessController.doPrivileged(new PrivilegedAction<Void>() {
            public Void run() {
                //这里会实例化一个ServiceLoader
                ServiceLoader<Driver> loadedDrivers = ServiceLoader.load(Driver.class);
                //得到一个迭代器(这个迭代器是包装了我们之前在ServiceLoader中实例化的一个LazyIterator)为外界提供了访问LazyIterator的方法,现在我们详细的分析
                 ...
                 */
                try{
                    //分析hasNext方法(for循环把我们的服务全部加载)
                    while(driversIterator.hasNext()) {
                        //ok,到这里你应该能想到下面就是加载服务的方法了,进行分析
        ...
        for (String aDriver : driversList) {
            try {
                ...
                // 可以看到这里是的参数为true所以会引起调用,也会被缓存到registeredDrivers中去
    ...
    private static Connection getConnection(
        ...
        /**
         * 下面就是遍历我们的之前所缓存的驱动,并获取连接,连接之后再分析把
         */
        for(DriverInfo aDriver : registeredDrivers) {
            // If the caller does not have permission to load the driver then
            // skip it.
