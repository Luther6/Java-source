...
class AnnotationInvocationHandler implements InvocationHandler, Serializable {
    ...
    // 构造函数的参数为注解的属性值,与注解类型。在注解类型中缓存了注解对象的必要属性,用来完成JDK的动态代理
    AnnotationInvocationHandler(Class<? extends Annotation> type, Map<String, Object> memberValues) {
        this.type = type;
        this.memberValues = memberValues;
    }
    // 再分析一下当前的InvocationHandler是如何匹配代理对象的方法
    public Object invoke(Object proxy, Method method, Object[] args) {
        // 这里获取到注解的代理对象的方法名,因为我们就是通过注解对象的属性名(也就是方法名)value来当作Map的key
        ...
        // Handle annotation member accessors,这里从缓存中通过key(方法名)获取到我们所需要的值(这些就是Java注解的实现原理)现在回到前面去
