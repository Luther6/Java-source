...
public class AnnotationParser {
    ...
    // 在这里的方法通过Class<? extends Annotation>泛型特性我们可以知道所有的注解都继承这个Annotation的接口
    private static Map<Class<? extends Annotation>, Annotation> parseAnnotations2(
        ...
        // 这里通过某种规则得到了该类对象有多少注解(至于什么规则我也还没搞清楚),根据我们的例子这里numAnnotations为1
        int numAnnotations = buf.getShort() & 0xFFFF;
        for (int i = 0; i < numAnnotations; i++) {
            //在这里就生成了我们的注解,所以有关细节需要进入到parseAnnotation2中了解
    ...
    private static Annotation parseAnnotation2(ByteBuffer buf,
                                              ...
                                              boolean exceptionOnMissingAnnotationClass,
                                              Class<? extends Annotation>[] selectAnnotationClasses) {
        //通过某种规则获得到该注解存在于常量池(到这里别糊涂了,这里的常量池指的是注解应用的那个类的常量池)
        ...
        try {
            try {
                ...
                // support obsolete early jsr175 format class files(这里是为了兼容早期的字节码文件)
        ...
        // 上面是检查一些异常情况,如果一切正常的话,那么程序将执行到这里
        AnnotationType type = null;
        try {
            // 从这里我们可以知道,我们的自定义的注解应该是在这里生成该注解类型并输出的,所以我们对其进行分析
            type = AnnotationType.getInstance(annotationClass);
        } catch (IllegalArgumentException e) {
            skipAnnotation(buf, false);
            return null;
        }
        // 获取当前注解的成员遍历的返回值类型与成员变量的方法对象的Map
        Map<String, Class<?>> memberTypes = type.memberTypes();
        Map<String, Object> memberValues =
            new LinkedHashMap<String, Object>(type.memberDefaults());

        int numMembers = buf.getShort() & 0xFFFF;
        // 用来遍历注解对象的属性,用于赋予我们给其赋的值,(AnnotationBais类中我们给@CustomerAnnotation的值)
        for (int i = 0; i < numMembers; i++) {
            ...
            if (memberType == null) {
                ...
                // 这从当前类对象AnnotationBasic的常量池找对该方法对象赋予的值(所以我们赋予的值,会在当前类的常量池中)
                                              ...
                                              final Map<String, Object> memberValues)
    {
        return AccessController.doPrivileged(new PrivilegedAction<Annotation>() {
            public Annotation run() {
                // 到这里你应该彻底的了解注解对象如何生成的了,在完成之前,需要重点对这个AnnoatationImvocationHandler的生成分析
